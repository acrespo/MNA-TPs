...
% CREO QUE ESTO ESTÁ MAL, LO QUE SE BUSCA NO ES LA MEDIA DE CADA BLOQUE?
Luego se procede a calcular la media aritm\'etica dentro de cada bloque. Para esto se utiliza la funci\'on $mean$, que aplicada a matrices devuelve un vector con las medias de cada columna. Pero dado que lo que se busca es la media dentro de cada bloque se le aplica $mean$ a la traspuesta de la matriz $b$. De esta manera se obtuvo un vector con el valor promedio para cada bloque, $m$, el resultado de la compresión.
